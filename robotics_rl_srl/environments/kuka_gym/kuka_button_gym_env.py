...
from robotics_rl_srl.environments.srl_env import SRLGymEnv
from robotics_rl_srl.state_representation.episode_saver import EpisodeSaver
from robotics_rl_srl.srl_zoo.preprocessing import getNChannels

from robotics_rl_srl.environments.kuka_gym import kuka
...
class KukaButtonGymEnv(SRLGymEnv):
    ...
    def __init__(self, urdf_root=pybullet_data.getDataPath(), renders=False, is_discrete=False, multi_view=False,
    ...
    def reset(self):
        ...
        self.button_uid = p.loadURDF("robotics_rl_srl/urdf/simple_button.urdf", [x_pos, y_pos, Z_TABLE])
    ...
    def step2(self, action):
        ...
        if self.srl_model != "raw_pixels":
            return self.getSRLState(self._observation), reward, done, distance

        return np.array(self._observation), reward, done, distance
